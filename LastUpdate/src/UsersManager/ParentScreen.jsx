...
export default function ParentScreen({ parent, onEnterProfile, onParentUpdated, onNavigate }) {
    ...
    const openEdit = (profile) => {
        ...
        // Keep the profile's actual avatar — even a shop avatar that isn't in
        // AVATAR_OPTIONS — so editing the name doesn't silently wipe a purchase.
                ...
                <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
                    <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-8 max-w-md w-full shadow-2xl">
                        ...
                        <div className="flex items-center gap-2 mb-3 text-sm text-gray-600 dark:text-gray-300">
                            <span>Selected:</span>
                            <span className="text-2xl">{formAvatar}</span>
                            {editingId && !AVATAR_OPTIONS.includes(formAvatar) && (
                                <span className="text-xs text-gray-400 dark:text-gray-500">(kept from the shop — pick below only to change it)</span>
                            )}
                        </div>
