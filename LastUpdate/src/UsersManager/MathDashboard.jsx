...
// Coin reward per correct answer, by difficulty.
// MUST stay in sync with POINTS_PER_LEVEL in server/controllers/resultController.js.
const POINTS_PER_LEVEL = { beginner: 3, intermediate: 7, advanced: 20 };

const REWARDS = [
    { label: 'Easy', coins: POINTS_PER_LEVEL.beginner, dot: 'bg-green-500', text: 'text-green-500 dark:text-green-400' },
    { label: 'Medium', coins: POINTS_PER_LEVEL.intermediate, dot: 'bg-yellow-500', text: 'text-yellow-500 dark:text-yellow-400' },
    { label: 'Hard', coins: POINTS_PER_LEVEL.advanced, dot: 'bg-red-500', text: 'text-red-500 dark:text-red-400' },
];

export default function MathDashboard({ profile, onNavigate }) {
    ...
    const allSubjects = [
        { id: 'addition', label: 'Addition', icon: '➕', minAge: 6 },
        { id: 'subtraction', label: 'Subtraction', icon: '➖', minAge: 6 },
    ...
    return (
        <div className="p-6">
            ...
            {/* Coins-per-correct-answer legend (compact) */}
            <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 px-4 py-3 flex items-center justify-between flex-wrap gap-3">
                <p className="text-sm font-semibold text-gray-600 dark:text-gray-300 flex items-center gap-1.5">
                    <span>💰</span> Coins per correct answer
                </p>
                <div className="flex items-center gap-2">
                    {REWARDS.map((r) => (
                        <div key={r.label} className="flex items-center gap-1.5 bg-gray-50 dark:bg-gray-700/50 rounded-lg px-2.5 py-1">
                            ...
                            <span className={`text-sm font-bold ${r.text}`}>⭐{r.coins}</span>
                        </div>
                    ))}
                </div>
            </div>
