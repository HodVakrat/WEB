...
export default function ManageUsers() {
    ...
    return (
        <div className="bg-gray-800 p-8 rounded shadow-md mx-auto my-16 w-fit text-white border border-gray-700">
            <h2 className="text-xl font-bold mb-6 text-center">Manage Users</h2>
            <table className="table-auto border-collapse w-full text-sm">
                <thead>
                    <tr className="bg-gray-700 text-left border-gray-600">
                ...
                <tbody>
                    {users.map(user => (
                        <tr key={user.id} className="hover:bg-gray-700 border border-gray-600">
                            {editingId === user.id ? (
                                <>
                                    <td className="border border-gray-600 px-4 py-2">{user.username}</td>
                                    <td className="border border-gray-600 px-4 py-2">
                                        <input className="border border-gray-600 rounded p-1 w-32 bg-gray-700 text-white" value={editForm.name}
                                            onChange={e => handleChange('name', e.target.value)} />
                                    </td>
                                    <td className="border border-gray-600 px-4 py-2">
                                        <input className="border border-gray-600 rounded p-1 w-44 bg-gray-700 text-white" value={editForm.email}
                                            onChange={e => handleChange('email', e.target.value)} />
                                    </td>
                                    <td className="border border-gray-600 px-4 py-2">
                                        <select className="border border-gray-600 rounded p-1 bg-gray-700 text-white" value={editForm.role}
                                            onChange={e => handleChange('role', e.target.value)}>
                                            <option>User</option>
                                            <option>Admin</option>
                                        </select>
                                    </td>
                                    <td className="border border-gray-600 px-4 py-2 flex gap-1">
